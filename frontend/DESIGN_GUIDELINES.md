...
**Details keep their data safe:** the field's internal key is generated from its label and never shown to the user, so two details can share a label without colliding, and a detail can't accidentally overwrite a category field either — there's nothing for the user to mistype. There is no name-length limit (the label is ordinary text, same as a category field's), but an item can hold at most 50 extra details. Details appear in the order they were added and keep their position across saves (unlike the old freeform-text version of this feature, which had no stable order).
